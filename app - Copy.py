...
with tab3:
    ...
    with st.form(key="add_recipe_form", clear_on_submit=True):
        ...
        new_name = st.text_input("Recipe Name*", placeholder="e.g. Dadi's Special Kheer")
        new_category = st.selectbox("Type of Dish (Compulsory)*", ["Select Type...", "Main Course", "Curry", "Starters", "Breakfast", "Dessert", "Snack", "Bread", "Side Dish", "Other"])
        
        st.markdown("<h4 class='form-section-header'>🍳 Ingredients & Cooking Method</h4>", unsafe_allow_html=True)
        new_ingredients = st.text_area("Ingredients Needed (comma-separated)*", placeholder="e.g. Rice, Milk, Sugar, Saffron, Almonds")
        new_procedure = st.text_area("Preparation Steps / Procedure*", placeholder="e.g. 1. Wash rice. 2. Boil milk and add rice. 3. Simmer until rice is cooked. 4. Add sugar and garnish with nuts.")
        ...
        with st.expander("Additional Notes (Optional)"):
            ...
            new_notes = st.text_area("Family Secrets & Tips (Notes)", placeholder="E.g. Serve chilled for best taste...")
